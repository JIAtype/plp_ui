...
def show_vader_sentiment_analysis():
    ...
    with st.spinner('Loading sentiment data...'):
        ...
        sentiment_counts_vader = load_data('data/vader_sentiment_counts.csv')
        sentiment_counts_vader = sentiment_counts_vader.set_index('sentiment_vader').astype(int).transpose()
        
        comments_per_creator_sentiment = load_data("data/comments_per_creator_sentiment_vader.csv")
        topic_sentiment_df = load_data('data/topic_sentiment_summary_vader.csv')
        topic_sentiment_matrix = load_data("data/topic_sentiment_matrix_vader.csv")
        ...
        grouped = load_data('data/topic_sentiment_per_creator_vader.csv')
        positive_comment_time_series = load_data("data/positive_comment_time_series_vader.csv")
        ...
        creator_aspect_sentiment = load_data("data/topic_sentiment_matrix_comparison_between_creators_vader.csv")
        creator_aspect_sentiment = creator_aspect_sentiment.T
        
        aspect_video_df = load_data("data/topic_sentiment_matrix_creator_vader.csv")
...
# Add sections for additional analysis
# def show_comparison_analysis():
#     st.header("Creator Comparison Analysis")
...
#     creators = sentiment_counts_vader.index.tolist()
    
#     col1, col2 = st.columns(2)
    
#     with col1:
#         creator1 = st.selectbox("Select first creator:", creators, index=0)
    
#     with col2:
#         # Get default index for second creator
#         default_index = 1 if len(creators) > 1 else 0
#         creator2 = st.selectbox("Select second creator:", creators, index=default_index)
...
#             'Sentiment': sentiment_counts_vader.columns,
#             creator1: sentiment_counts_vader.loc[creator1].values,
#             creator2: sentiment_counts_vader.loc[creator2].values
#         })
        
#         # Calculate percentages
#         total1 = sentiment_counts_vader.loc[creator1].sum()
#         total2 = sentiment_counts_vader.loc[creator2].sum()
        
#         compare_pct_df = pd.DataFrame({
#             'Sentiment': sentiment_counts_vader.columns,
#             f"{creator1} (%)": (sentiment_counts_vader.loc[creator1].values / total1 * 100).round(1),
#             f"{creator2} (%)": (sentiment_counts_vader.loc[creator2].values / total2 * 100).round(1)
#         })
        
#         # Create comparison visualization
#         tab1, tab2 = st.tabs(["Absolute Numbers", "Percentages"])
...
#                 y=[creator1, creator2],
#                 barmode='group',
#                 title=f"Sentiment Comparison: {creator1} vs {creator2}",
#                 labels={'value': 'Number of Comments', 'variable': 'Creator'}
#             )
...
#             # Improve hover information
#             fig.update_traces(
#                 hovertemplate="<b>Sentiment:</b> %{x}<br><b>%{data.name}:</b> %{y}<extra></extra>"
#             )
...
#                 compare_pct_df, 
#                 x='Sentiment', 
#                 y=[f"{creator1} (%)", f"{creator2} (%)"],
#                 barmode='group',
#                 title=f"Sentiment Comparison (Percentage): {creator1} vs {creator2}",
#                 labels={'value': 'Percentage', 'variable': 'Creator'}
#             )
...
#             # Improve hover information
#             fig.update_traces(
#                 hovertemplate="<b>Sentiment:</b> %{x}<br><b>%{data.name}:</b> %{y:.1f}%<extra></extra>"
#             )
            
#             st.plotly_chart(fig, use_container_width=True)
        
#         # Topic comparison between creators
#         st.subheader("Topic Sentiment Comparison")
        
#         # Extract topic sentiment for selected creators
#         topic_sentiment1 = topic_sentiment_matrix_transposed.loc[creator1]
#         topic_sentiment2 = topic_sentiment_matrix_transposed.loc[creator2]
        
#         # Combine into a comparison dataframe
#         topic_compare = pd.DataFrame({
#             'Topic': topic_sentiment_matrix_transposed.columns,
#             creator1: topic_sentiment1.values,
#             creator2: topic_sentiment2.values,
#             'Difference': topic_sentiment1.values - topic_sentiment2.values
#         })
        
#         # Sort by absolute difference
#         topic_compare['Abs_Diff'] = topic_compare['Difference'].abs()
#         topic_compare = topic_compare.sort_values('Abs_Diff', ascending=False).drop('Abs_Diff', axis=1)
...
#             y=[creator1, creator2],
#             barmode='group',
#             title=f"Topic Sentiment Comparison: {creator1} vs {creator2}",
#             labels={'value': 'Sentiment Score', 'variable': 'Creator'}
#         )
...
#         # Improve hover information
#         fig.update_traces(
#             hovertemplate="<b>Topic:</b> %{x}<br><b>%{data.name}:</b> %{y:.2f}<extra></extra>"
#         )
        
#         st.plotly_chart(fig, use_container_width=True)
        
#         # Show topics with largest differences
#         st.subheader("Topics with Largest Sentiment Differences")
        
#         top_diff = topic_compare.head(5)
...
#             title=f"Top 5 Topics with Largest Sentiment Differences",
#             labels={'Difference': 'Sentiment Difference', 'Topic': 'Topic'}
#         )
        
#         fig.update_layout(
#             xaxis_title="Topic",
#             yaxis_title=f"Difference ({creator1} - {creator2})",
#             xaxis_tickangle=-45,
#             hoverlabel=dict(
#                 bgcolor="white",
#                 font_size=12,
...
#         # Improve hover information
#         fig.update_traces(
#             hovertemplate="<b>Topic:</b> %{x}<br><b>Difference:</b> %{y:.2f}<extra></extra>"
#         )
...
#             get_download_link(
#                 topic_compare,
#                 f"{creator1}_vs_{creator2}_topic_comparison.csv",
#                 "📥 Download topic comparison data as CSV"
#             ),
#             unsafe_allow_html=True
#         )
...
#     st.write("Analyze sentiment patterns in YouTube video comments across different creators and topics.")
    
#     # Create sidebar navigation
#     st.sidebar.title("Navigation")
#     page = st.sidebar.radio(
#         "Select Analysis:",
#         ["VADER Sentiment Analysis", "Creator Comparison", "Video Search"]
#     )
    
#     # Show the selected page
#     if page == "VADER Sentiment Analysis":
#         show_vader_sentiment_analysis()
#     elif page == "Creator Comparison":
...
#     st.sidebar.info(
#         """
#         This dashboard visualizes sentiment analysis of YouTube comments using VADER.
        
#         Data is processed to extract topics and sentiment from viewer comments.
#         """
#     )
...
#         st.write("""
#         - **Overview**: See general sentiment distribution across creators
#         - **By Creator**: Analyze sentiment for specific content creators
#         - **By Topic**: Explore sentiment patterns across different topics
#         - **Time Series**: Track sentiment changes over time
#         - **Aspect-Based**: Deep dive into aspect-specific sentiment 
#         - **Creator Comparison**: Compare sentiment between creators
#         - **Video Search**: Find and analyze specific videos
        
#         Use the download buttons to export data for further analysis.
#         """)